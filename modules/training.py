...
def run_partition(
    model, epoch, tensorboard, optimizer, loader, partition, training, metrics
):
    if hasattr(model, "on_epoch_start"):
        model.on_epoch_start(
            epoch=epoch, tensorboard=tensorboard, partition=partition, loader=loader
        )
    ...
    for (batch_data, batch_labels) in tqdm(loader, desc="{} batches".format(partition)):
        ...
        # forward pass
        batch_losses, info = model.compute_loss(
            inputs=batch_data, labels=batch_labels, grad_enabled=training
        )
        ...
        for metric in metrics:
            metric.on_iteration_end(
                info=info, batch_labels=batch_labels, partition=partition
            )
...
def train(
    ...
    tensorboard.add_text(
        "script arguments table", make_markdown_table_from_dict(vars(args_to_log))
    )
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % vis_iter == 0 and hasattr(model, "visualize"):
            visualizations = model.visualize(
                train_loader, val_loader, tensorboard=tensorboard, epoch=epoch
            )
